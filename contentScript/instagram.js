...
chrome.storage.local.get(["state"], function (result) {
  ...
  if (result.state.timestate.deadline - new Date()) isBlockForCountDown = true;
  if (isBlockForTodo && !isBlockForCountDown)
    blockForTodo(result.state.listTodo, result.state.setting.instagram);
  if (!isBlockForTodo && isBlockForCountDown)
    blockForCountDown(result.state.setting.instagram);
  if (isBlockForTodo && isBlockForCountDown)
    blockBoth(result.state.listTodo, result.state.setting.instagram);
});
